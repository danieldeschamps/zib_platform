...
LONG CSerial::Open (LPCTSTR lpszDevice, DWORD dwInQueue, DWORD dwOutQueue, bool fOverlapped)
{
	...
	if (m_hFile == INVALID_HANDLE_VALUE)
	{
		...
		TRACE(DBG_LVL_DEBUG, "CSerial::Open - Unable to open port\n");
		return m_lLastError;
	}
...
LONG CSerial::Write (const void* pData, size_t iLen, DWORD* pdwWritten, LPOVERLAPPED lpOverlapped, DWORD dwTimeout)
{
	...
	*pdwWritten = 0;
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		TRACE(DBG_LVL_DEBUG, "CSerial::Write - Device is not opened\n");
		return m_lLastError;
	}
	...
	if (!::WriteFile(m_hFile,pData,iLen,pdwWritten,lpOverlapped))
	{
		...
		if (lpOverlapped == &ovInternal)
		{
			...
			case WAIT_OBJECT_0:
				// The overlapped operation has completed
				if (!::GetOverlappedResult(m_hFile,lpOverlapped,pdwWritten,TRUE)) // todo: verify - dds: changed this to TRUE
				{
					// Set the internal error code
					m_lLastError = ::GetLastError();
	...
	*pdwRead = 0;
	...
	if (!::ReadFile(m_hFile,pData,iLen,pdwRead,lpOverlapped))
	{
		...
		if (lpOverlapped == &ovInternal)
		{
			...
			case WAIT_OBJECT_0:
				// The overlapped operation has completed
				if (!::GetOverlappedResult(m_hFile,lpOverlapped,pdwRead,TRUE)) // todo: verify - dds: changed this to TRUE
				{
					// Set the internal error code
					m_lLastError = ::GetLastError();
