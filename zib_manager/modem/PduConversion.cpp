...
int decode_address_number( char* szAddress ) // Inverts the chars positions two by two.
{
	int iSize = strlen( szAddress );
	for(int i = 0 ; 2*i < iSize ; i++)
	{
		char cBuffer;
		cBuffer = szAddress[2*i];
		szAddress[2*i] = szAddress[2*i+1]; 
		szAddress[2*i+1] = cBuffer;
	}

	if( szAddress[iSize-1] == 'F' )
	{
		szAddress[iSize-1] = '\0';
	}
