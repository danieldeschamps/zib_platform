...
unsigned char * NEAR SRAMalloc(NEAR unsigned char nBytes)
{
    ...
    while (1)
    {
        #ifdef ENABLE_DEBUG
            TRACE(" check " );
            TRACE_CHAR( (BYTE)((WORD)pHeap>>8) );
            TRACE_CHAR( (BYTE)((WORD)pHeap&0xff) );
        #endif
...
void SRAMfree(unsigned char * NEAR pSRAM)
{
    ...
    #ifdef ENABLE_DEBUG
        TRACE("\r\nFree " );
        TRACE_CHAR((*(SALLOC *)(pSRAM - 1)).byte - 1);
        TRACE(" at " );
        TRACE_CHAR( (BYTE)((WORD)(pSRAM - 1)>>8) );
        TRACE_CHAR( (BYTE)((WORD)(pSRAM - 1)&0xff) );
    #endif
}
