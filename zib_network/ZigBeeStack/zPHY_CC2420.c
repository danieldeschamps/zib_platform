...
{
		// dds:
		// CCP2 is configured to capture on rising edge of RB3(FIFOP)
		// FIFOP will go high when the last byte of a new packet is received -> Read one byte from RXFIFO and and FIFOP will go back to low
		// FIFOP will go high when the number of unread bytes in the RXFIFO exceeds the threshold programmed
		/* CC2420 pg 33:
		When address recognition is enabled, data should not be read out of the RXFIFO before the address is completely received, 
		since the frame may be automatically flushed by CC2420 if it fails address recognition. This may be handled by using the 
		FIFOP pin, since this pin does not go high until the frame passes address recognition.
		*/
    if(CCP2IF)
    {
        if(CCP2IE)
        {
            ...
            if(RxData.inUse == 0)
            {
                if(PHY_FIFO == 0) // If FIFO signal goes low while FIFO signal is high, cc2420 is signaling a RXFIFO overflow [cc2420, pg33]
                {
                    // Flush the RX FIFO
                    iPHY_CSn_0();
                    SPIPut(REG_RXFIFO | CMD_READ);
            ...
            if(w.Val < RxBytesRemaining) // enough room left in the buffer?
            {
...
//                        }
                ...
                if(w.bits.b7 == 0) // get the crc byte
                {
