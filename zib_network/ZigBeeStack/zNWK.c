...
#define BROADCAST_JITTER            ((MAC_LONG_ADDR_BYTE0 & (nwkcMaxBroadcastJitter-1)) * ONE_SECOND / 1000)
    ...
    #define RREQ_BROADCAST_JITTER   (2 * nwkcMinRREQJitter * ONE_SECOND / 1000)
#else
...
typedef struct _NIB
{
    BYTE nwkBCSN;
        ...
        ROM NEIGHBOR_RECORD* nwkRouteTable;
...
FinishBroadcastPacket:
                ...
                {
                    //TRACE("NWK: All broadcast messages complete\r\n" );
...
SendUpNoRouteAvailable:
                ...
                if (i == NUM_BUFFERED_ROUTING_MESSAGES)
                {
                    ...
                    {
                        //TRACE("NWK: All route discoveries complete\r\n" );
...
HandleRouteRequest:
...
#endif // I_AM_RFD
                    {
                            ...
                            if (nwkStatus.flags.bits.bCanRoute)
                            {
                                ...
                                else if (i == NWK_COMMAND_ROUTE_REPLY)
                                {
        ...
        #endif // USE_TREE_ROUTING_ONLY
                                    {
                                        ...
                                        {
                                            //rrep.pathCost += CONSTANT_PATH_COST; TODO This is in the spec algorithm, but isn't this already in path cost from the destination???
                                            forwardRouteReply = TRUE;
                                        }
                                    }
                        ...
                        else
                        {
                            ...
                            {
                                routingStatus = GetRoutingAddress( FALSE, nwkDestinationAddress, nwkFrameControlLSB.bits.routeDiscovery, &macAddress ); // dds: here's where we route
...
//                break;
            ...
            case MLME_ASSOCIATE_indication:
                ...
                if (NWKLookupNodeByLongAddr( &(params.MLME_ASSOCIATE_indication.DeviceAddress) ) != INVALID_NEIGHBOR_KEY)
                {
...
//                params.NLME_LEAVE_confirm.DeviceAddress.v[0] = MAC_LONG_ADDR_BYTE0;
//                params.NLME_LEAVE_confirm.DeviceAddress.v[1] = MAC_LONG_ADDR_BYTE1;
//                params.NLME_LEAVE_confirm.DeviceAddress.v[2] = MAC_LONG_ADDR_BYTE2;
//                params.NLME_LEAVE_confirm.DeviceAddress.v[3] = MAC_LONG_ADDR_BYTE3;
//                params.NLME_LEAVE_confirm.DeviceAddress.v[4] = MAC_LONG_ADDR_BYTE4;
//                params.NLME_LEAVE_confirm.DeviceAddress.v[5] = MAC_LONG_ADDR_BYTE5;
//                params.NLME_LEAVE_confirm.DeviceAddress.v[6] = MAC_LONG_ADDR_BYTE6;
//                params.NLME_LEAVE_confirm.DeviceAddress.v[7] = MAC_LONG_ADDR_BYTE7;
...
//                break;
            ...
            case MLME_SCAN_confirm:
                    ...
                    if (params.MLME_SCAN_confirm.ScanType == MAC_SCAN_ENERGY_DETECT)
                    {
                        ...
                        {
	                    #ifdef DEBUG_ENERGY_DETECT
	                        TRACE("NWK: *MLME_SCAN_confirm* The ResultListSize is zero\r\n");
	                    #endif
NetworkStartupFailure:
                        ...
                        if ((nwkStatus.discoveryInfo.channelList = (CHANNEL_INFO *)SRAMalloc( 16 * sizeof(CHANNEL_INFO) )) == NULL)
                        {
                        	...
                        	#ifdef DEBUG_ENERGY_DETECT
                            TRACE("NWK: *MLME_SCAN_confirm* Could not allocate channelList\r\n");
                        	#endif
                        ...
                        #ifdef DEBUG_ENERGY_DETECT
                        TRACE("NWK: Enumerating channels after scan\r\n");
                        #endif
                        for (i=0; i<32; i++)
                        {
                            if (nwkStatus.lastScanChannels.Val & 0x00000001)
                            {
                                if (!(params.MLME_SCAN_confirm.UnscannedChannels.Val & 0x00000001))
                                {
	                                ...
	                                TRACE_INT(*ptr);
	                                TRACE(" THRSLD=");
	                                TRACE_INT(MAX_ENERGY_THRESHOLD);
	                                TRACE("\r\n");
	                            	#endif
                        ...
                        if (!nwkStatus.discoveryInfo.numChannels)
                        {
	                    	...
	                    	#ifdef DEBUG_ENERGY_DETECT
                            TRACE("NWK: *MLME_SCAN_confirm* There were no channels with energy below the threshold\r\n");
	                    	#endif
...
StartActiveScan:
                    ...
                    if (params.MLME_SCAN_confirm.ScanType == MAC_SCAN_ACTIVE_SCAN)
                    {
                        ...
                        {
	                    #ifdef DEBUG_SCAN_ACTIVE_SCAN
	                        TRACE("NWK: The scan was not successfull. status=");
	                        TRACE_CHAR(params.MLME_SCAN_confirm.status);
	                        TRACE("\r\n");
	                    #endif
                    ...
                    #ifdef DEBUG_SCAN_ACTIVE_SCAN
                        TRACE("NWK: The scan was successfull\r\n");
                    #endif
...
//                    #endif
                    {
                        if (params.NLDE_DATA_request.SecurityEnable)
                        {
                            // TODO Send to security suite
                        }
                    }
...
void AddChildNode( void )
{
    ...
    if (params.MLME_ASSOCIATE_indication.CapabilityInformation.CapBits.DeviceType == DEVICE_IEEE_RFD)
    {
        ...
        //TRACE("Joining node " );
        //TRACE_CHAR( currentNeighborRecord.shortAddr.byte.MSB );
        //TRACE_CHAR( currentNeighborRecord.shortAddr.byte.LSB );
        //TRACE(" as end device\r\n" );
...
//    if ((address->v[0] == MAC_LONG_ADDR_BYTE0) &&
//        (address->v[1] == MAC_LONG_ADDR_BYTE1) &&
//        (address->v[2] == MAC_LONG_ADDR_BYTE2) &&
//        (address->v[3] == MAC_LONG_ADDR_BYTE3) &&
//        (address->v[4] == MAC_LONG_ADDR_BYTE4) &&
//        (address->v[5] == MAC_LONG_ADDR_BYTE5) &&
//        (address->v[6] == MAC_LONG_ADDR_BYTE6) &&
//        (address->v[7] == MAC_LONG_ADDR_BYTE7))
...
//        NIB.nwkSecureAllFrames                  = DEFAULT_nwkSecureAllFrames;
//        NIB.nwkSecurityLevel                    = DEFAULT_nwkSecurityLevel;
